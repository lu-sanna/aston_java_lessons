...
public class PageObjectsMtsBy {
    ...
    public void checkPaymentSystemsLogos() {
        ...
        for (String system : systems) {
            ...
            assertTrue(logo.isDisplayed(), "Логотип " + system + " не отображается");
        }
        takeScreenshot();
    }
    ...
    @Step("Проверка функциональности формы оплаты")
    ...
    @Step("Проверка плейсхолдеров для всех услуг")
    ...
    @Step("Проверка модального окна оплаты услуг связи")
    public void testConnectionPaymentWithModal() {
        ...
        // Делаем скриншот перед переходом в iframe
        takeScreenshot();

        continueButton.click();

        // Ожидание появления iframe с платежной формой
        ...
        try {
            // Увеличиваем время ожидания загрузки формы внутри iframe
            WebDriverWait iframeWait = new WebDriverWait(driver, Duration.ofSeconds(15));

            // Ждем загрузки основных элементов формы
            ...
            List<WebElement> amountElements = driver.findElements(
                    By.xpath("//*[contains(text(), '5.00') and contains(text(), 'BYN')]")
            );
